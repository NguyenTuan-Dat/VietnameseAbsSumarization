...
class Trainer(object):
                ...
                if self.n_gpu == 0 or (i % self.n_gpu == self.gpu_rank):
                    ...
                    if accum == self.grad_accum_count:
                        ...
                        if step % 100 == 0:
                            ...
                            list_xent_valid.append(self.validate(valid_iter=valid_iter, step=step).xent())

                            self.save_plot(list_xent_train, list_xent_valid,
                                           "/content/drive/MyDrive/Colab Notebooks/Models/PreSumm/save_plot_Baseline_" + str(
                                               step) + ".txt")
    ...
    def save_plot(self, list_train_xent, list_valid_xent, file_path):
        file_plot = open(file_path, "w")

        str_list_train_xent = str(list_train_xent[0])
        for train_xent in list_train_xent[1:]:
            str_list_train_xent += "," + str(train_xent)

        str_list_valid_xent = str(list_valid_xent[0])
        for valid_xent in list_valid_xent[1:]:
            str_list_valid_xent += "," + str(valid_xent)

        file_plot.write(str_list_train_xent + "\n" + str_list_valid_xent)
